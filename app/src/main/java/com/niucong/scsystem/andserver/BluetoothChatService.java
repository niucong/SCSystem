...
package com.niucong.scsystem.andserver;
...
//            UUID.fromString("fa87c0d0-afac-11de-8a39-0800200c9a66");
    // UUID，蓝牙建立链接需要的
//    private final UUID MY_UUID_SECURE = UUID.randomUUID();
...
//                    Log.e(TAG, "run: read " + new String(buffer, 0, bytes));
                    ...
                    if (MainActivity.result.endsWith("}") && MainActivity.result.contains("method")
                            && MainActivity.result.contains("data") && MainActivity.result.contains("updateTime")) {
                    ...
                    //TODO 连接丢失显示并重新开始连接
                    sendMessageToUi(BLUE_TOOTH_TOAST, "设备连接失败/传输关闭");
        ...
        public void write(byte[] buffer) {
            try {
                out.write(buffer);
                //TODO 到到UI显示
                uiHandler.obtainMessage(BLUE_TOOTH_WRAITE, -1, -1, buffer)
                        .sendToTarget();
            } catch (IOException e) {
    ...
    class ConnectThread extends Thread {
        ...
        public ConnectThread(BluetoothDevice device) {
            ...
            try {
                ...
                sendMessageToUi(BLUE_TOOTH_TOAST, "连接失败，请重新连接");
            }
            socket = mSocket;
        }
